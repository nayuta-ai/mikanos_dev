...
CallApp:  ; int CallApp(int argc, char** argv, uint16_t ss,
    ...
    push rcx ; RIP
    o64 retf
    ; If the application is terminated, it will not come here.
...
SyscallEntry: ; void SyscallEntry(void);
    ...
    push rax ; Save the number of systemcall

    mov rcx, r10
    and eax, 0x7fffffff
    mov rbp, rsp

    ; Preparation for executing system calls on the stack for OS
    ...
    ; rbx, r12-r15 are callee-saved, so they are not saved by the caller
    ; rax is for return values, so it is not saved by the caller

    mov rsp, rbp

    pop rsi ; Return the number of systemcall
    cmp esi, 0x80000002
    je .exit
