/**
 * @file main.cpp
 * The file containing the kernel main program.
 */
...
void InputTextWindow(char c) {
  ...
  if (c == '\b' && text_window_index > 0) {
    ...
    --text_window_index;
    FillRectangle(*text_window->InnerWriter(), pos(), {8, 16},
                  ToColor(0xffffff));
    DrawTextCursor(true);
...
extern "C" void KernelMainNewStack(
  ...
  while (true) {
    ...
    FillRectangle(*main_window->InnerWriter(), {20, 4}, {8 * 10, 16},
                  {0xc6, 0xc6, 0xc6});
    ...
    switch (msg->type) {
      ...
      case Message::kTimerTimeout:
        if (msg->arg.timer.value == kTextboxCursorTimer) {
          __asm__("cli");
          timer_manager->AddTimer(Timer{msg->arg.timer.timeout + kTimer05Sec,
